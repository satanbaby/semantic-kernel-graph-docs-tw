...
本教程將教您如何在 SemanticKernel.Graph 中使用狀態。您將學習如何使用 `KernelArguments` 和 `GraphState`、管理節點之間的數據流，以及利用強大的狀態管理功能。

## 您將學到什麼

* 狀態如何在圖形中流動
* 使用 `KernelArguments` 進行輸入和輸出
* 使用 `GraphState` 進行增強型狀態管理
* 狀態驗證和版本控制
* 狀態設計的最佳實踐
...
SemanticKernel.Graph 中的狀態表示在工作流中流動的數據。它就像一個背包，從節點傳遞到節點，在整個執行過程中收集和傳遞信息。
...
## 使用 GraphState 的增強型狀態管理

### 什麼是 GraphState？

`GraphState` 是 `KernelArguments` 的增強包裝器，提供了額外功能，如：
* 狀態版本控制和驗證
* 執行歷史追蹤
* 元數據管理
* 序列化功能
...
builder.AddGraphSupport(options =>
{
    ...
    options.Checkpointing.Interval = 3; // 每 3 個節點檢查點
});
...
```
**解決方案**：確保節點使用 `StoreResultAs()` 或手動狀態分配正確存儲結果。

#### 類型轉換錯誤
```
System.InvalidCastException: Unable to cast object of type 'System.String' to type 'System.Int32'.
```
**解決方案**：使用類型安全的訪問方法，如 `TryGetValue` 進行類型檢查，或在轉換前驗證類型。
...
* **[高級模式](patterns/index.md)**：發現複雜的狀態管理模式

## 概念和技術

本教程涵蓋了幾個關鍵概念：

* **狀態**：在圖形中流動的數據，在執行步驟中保持上下文
* **KernelArguments**：保存鍵值對的基本狀態容器
* **GraphState**：具有版本控制、驗證和元數據的增強型狀態包裝器
* **狀態流**：數據如何在節點之間移動並在執行期間變換
* **狀態驗證**：確保整個工作流中的數據完整性和類型安全

## 先決條件和最小配置

要完成本教程，您需要：
* **SemanticKernel.Graph** 包已安裝並配置
* **LLM 提供者**具有有效的 API 密鑰
* **基本理解**來自第一個教程的圖形概念
* **.NET 8.0+** 運行時環境

## 另請參閱

* **[首個圖形教程](first-graph.md)**：構建您的第一個完整圖形工作流
